...
class TestMvameldingMerknad(TransactionCase):
    ...
    def test_blank_merknad_avvises(self):
        ...
        Dekker mer enn mellomrom: btrim() uten tegnsett trimmer kun ' ', så
        en beskrivelse på "\n" eller "\t" slapp tidligere gjennom
        constrainten og ble først stoppet av submit-gaten.
        """
        for blank in ("   ", "\n", "\t", " \r\n "):
            with self.subTest(blank=repr(blank)):
                with self.assertRaises(Exception):
                    with self.env.cr.savepoint():
                        self._merknad(tekst=blank)
